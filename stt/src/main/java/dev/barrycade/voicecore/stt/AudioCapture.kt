...
internal class AudioCapture(
    ...
    fun start() {
        synchronized(stateLock) {
            ...
            SttLogger.pcm("AudioCapture.start() — invoking AudioRecord.startRecording() synchronously")
            try {
                ar.startRecording()
            } catch (e: Exception) {
                ar.release()
            ...
            SttLogger.pcm("AudioRecord.startRecording() returned OK")
            ...
            SttLogger.pcm("Capture started [Rate: $sampleRate, ReadBuffer: $bufferSizeSamples samples, AudioRecordBuffer: $finalBufferSizeInBytes bytes]")
        }
    }
